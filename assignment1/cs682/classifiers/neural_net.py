...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    scores_nom = scores_exp / np.sum(scores_exp, axis=1).reshape(N, -1)
    loss = np.sum(-1 * np.log(scores_nom[np.arange(N), y].reshape(N, 1)))
    loss /= N
    ...
    #############################################################################
    dscore = np.zeros_like(scores_nom)
    dscore[np.arange(N), y] = -1
    dscore = dscore + scores_nom
    dW2= np.dot(hidden.T, dscore)/N
    dW2 += 2 * W2 * reg
    grads['W2']=dW2
    grads['b2'] =np.sum(dscore,axis=0)/N
    dW1 = np.dot(dscore, W2.T) / N
    dW1[hidden == 0] = 0
    grads['W1']=np.dot(X.T, dW1) +2 * W1 * reg
